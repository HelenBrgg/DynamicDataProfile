...
public interface PartitioningStrategy {
    ...
    default List<Partition> partitionStream(String attribute, int numWorkers, Stream<Integer> positions) {
        IntSummaryStatistics stats = positions.collect(Collectors.summarizingInt(Integer::intValue)); // TODO
        return this.partitionRange(attribute, numWorkers, stats.getMin(), stats.getMax());
    }
    ...
    default void partitionTable(Table table, BiConsumer<Partition, Stream<ValueWithPosition>> consumer) {
        ...
        for (int colIndex = 0; colIndex < table.attributes.size(); colIndex++) {
            ...
            for (Partition partition : this.partitionRange(attribute, table.positions.stream())) {
                ...
                Stream<ValueWithPosition> partitionValues = table.streamColumn(col, partition.rangeBegin,
                        partition.rangeEndInclusive);
